...
from bokeh.models.widgets import Slider, TextInput, Tabs, Panel 
...
source = ColumnDataSource(data=dict(x=wave[2:-3], y=snr[2:-3], size=[1,2,3,4,5], desc=['U','B','V','R','I']))
...
snr_plot.line('x', 'y', source=source, line_width=3, line_alpha='size') 
...
difflimit = Slider(title="Diff. Limit Wavelength", value=0.5, start=0.2, end=1.)
subexposures = Slider(title="Number of Subexposures", value=3, start=1, end=10., step=1.)
uv_pixel = Slider(title="UV Pixel Size", value=0.016, start=0.010, end=0.05, step=0.001)
opt_pixel = Slider(title="Vis Pixel Size", value=0.016, start=0.010, end=0.05, step=0.001)
nir_pixel = Slider(title="NIR Pixel Size", value=0.04, start=0.040, end=0.10, step=0.01)
exp_inputs = Panel(child=VBoxForm(children=[difflimit, subexposures, uv_pixel, opt_pixel, nir_pixel]), title="Expert")
...
def here_is_a_callback():
    print 'INSIDE THE PERIODIC CALLBACK', aperture.value, magnitude.value, exptime.value 
    # Get the current slider values
    a = aperture.value 
    m = magnitude.value
    e = exptime.value
...
easy_inputs = Panel(child=inputs, title="Basics")
tabs = Tabs(tabs=[ easy_inputs, exp_inputs ])
curdoc().add_root(HBox(children=[tabs, snr_plot], width=1800))
